...
public class addOrderCommand implements Command {
    ...
    public void execute() {
        ...
        // Создание заказа
        adminService.addOrder(plannedDate, price);

        System.out.println("Заказ успешно создан!");

        // Предложение назначить ресурсы
        offerAssignResources(scanner, adminService.getLastAddedOrderId());
    }
    ...
    private void assignResourcesToOrder(Scanner scanner, int orderId) {
        ...
        adminService.getMastersAlphabetically().forEach(m ->
        ...
        adminService.getAllGarageSpots().forEach(s ->
        ...
        // Назначение ресурсов
        boolean success = adminService.assignResourcesToOrder(orderId, masterId, spotId);
